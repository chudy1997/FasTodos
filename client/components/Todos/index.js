...
class Todos extends Component {
    ...
    createTodos = () => {
        ...
        return this.props.todos.filter(todo => todo.categoryId === this.getCategoryId()).map(todo =>
            <li style={{backgroundColor:this.props.colorMap[this.getCategoryId()%Object.keys(this.props.colorMap).length]}} key={(id++)} id={id} className={this.props.chosenTodoId == id ? 'todo chosen-todo': 'todo'} onClick={this.chooseTodo}>
                {todo.text}
            </li>
        );
    }
